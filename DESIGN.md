...
version: alpha
name: Linear
description: "A near-black product-focused marketing canvas built around #010102 (the deepest dark surface of any tool in this collection), light gray text (#f7f8f8), and the signature Linear orange (#f97316) used as the single chromatic accent. The system reads as software-craft documentation: dense, technical, and quietly luxurious. Display type is set in the Linear custom sans (SF Pro Display fallback) at 500–700 with measured negative tracking. Cards live as charcoal panels (#0f1011) with hairline borders. The accent orange appears on the brand mark, focus rings, and a few intentional CTAs — never decoratively. Page rhythm leans on product UI screenshots framed in dark panels rather than atmospheric color."

colors:
  primary: "#f97316"
  on-primary: "#ffffff"
  primary-hover: "#fb923c"
  primary-focus: "#ea6720"
  ink: "#f7f8f8"
  ink-muted: "#d0d6e0"
  ink-subtle: "#8a8f98"
  ink-tertiary: "#62666d"
...
The single chromatic accent is **Linear orange** `{colors.primary}` (#f97316) — used on the brand mark, focus rings, and the primary CTA button. A lighter hover state (`{colors.primary-hover}` #fb923c) and a focus-tinted variant (`{colors.primary-focus}` #ea6720) extend the same hue. Linear avoids saturated greens, purples, reds, etc. on the marketing canvas — the only semantic color is `{colors.semantic-success}` (#27a644) for status pills and the rare success indicator.
...
- **Orange brand accent** (`{colors.primary}` #f97316) — used scarcely on brand mark, focus, and the primary CTA.
...
### Brand & Accent
- **Orange** ({colors.primary}): The signature Linear accent — primary CTA, brand mark, link emphasis.
- **Orange Hover** ({colors.primary-hover}): Lighter orange (#fb923c) — hovered state of the primary CTA.
- **Orange Focus** ({colors.primary-focus}): Focus-ring tint (#ea6720) — focused inputs, focused buttons.
- **Brand Secure** ({colors.brand-secure}): Muted orange-brown (#c47c3a) — used in "Linear Security" surfaces.
...
**`button-primary`** — Orange CTA. The default primary CTA across all pages.
- Background `{colors.primary}`, text `{colors.on-primary}`, type `{typography.button}`, padding 8px 14px, rounded `{rounded.md}`.
- Pressed state lives in `button-primary-pressed` (background shifts to `{colors.primary-focus}`).
- Hover state lives in `button-primary-hover` (background shifts to `{colors.primary-hover}` lighter orange).
...
- Use `{colors.primary}` orange ONLY for: brand mark, primary CTA, focus ring, link emphasis.
...
- Don't introduce a second chromatic accent (purple, pink, green for marketing).
...
6. Treat orange as scarce: brand mark, primary CTA, focus, link emphasis.
